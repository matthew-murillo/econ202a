...
for n=1:p.maxit
    ...
    dVb(1,:) = p.mu(p.w*zz(1,:) + r.*aa(1,:)); % a>=a_min is enforced (borrowing constraint)
    dVf(end,:) = p.mu(p.w*zz(end,:) + r.*aa(end,:)); % a<=a_max is enforced which helps stability of the algorithm
    ...
    Ib = sb<0;
    I0 = 1-If-Ib;
    dV0 = p.mu(p.w*zz + r.*aa); % If sf<=0<=sb, set s=0
