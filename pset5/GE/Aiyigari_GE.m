...
%end
        ...
        %% 3-4. Guess an initial value of the value function
            ...
            r = r0;
            z_ave = (p.z(1)*p.lambda(2) + p.z(2)*p.lambda(1))/(p.lambda(1)+p.lambda(2));
            KD = (p.alpha*p.A/(r + p.d))^(1/(1-p.alpha))*z_ave;
            p.w = (1-p.alpha)*p.A*KD.^p.alpha*z_ave^(-p.alpha);

            v0 = p.u(p.w*zz + r.*kk)./p.rho; 
            V = v0;
...
nr =1
        for nr=1:p.Nr
            ...
            z_ave = (p.z(1)*p.lambda(2) + p.z(2)*p.lambda(1))/(p.lambda(1)+p.lambda(2));
            KD = (p.alpha*p.A/(r_r(nr) + p.d))^(1/(1-p.alpha))*z_ave;
            p.w = (1-p.alpha)*p.A*KD.^p.alpha*z_ave^(-p.alpha);
...
for n=1:p.maxit
    ...
    dVb(1,:) = p.mu(p.w*zz(1,:) + r.*kk(1,:)); % k>=a_min is enforced (borrowing constraint)
    dVf(end,:) = p.mu(p.w*zz(end,:) + r.*kk(end,:)); % k<=a_max is enforced which helps stability of the algorithm
    ...
    Ib = sb<0;
    I0 = 1-If-Ib;
    dV0 = p.mu(p.w*zz + r.*kk); % If sf<=0<=sb, set s=0
...
% Normalization
g_sum = g_stacked'*ones(2*p.I,1)*dk;
...
g_sum_eigs = g_stacked_eigs'*ones(2*p.I,1)*dk;
g_stacked_eigs = g_stacked_eigs./g_sum_eigs;
        ...
        % Notes: Each matrix has dimensions p.I*2(u,e)*nr
            ...
            g_r(:,:,nr) = gg;
            adot(:,:,nr) = p.w*zz + r.*kk - c;
            V_r(:,:,nr) = V;
            dV_r(:,:,nr) = dV_upwind;
            c_r(:,:,nr) = c;
            
            S(nr) = gg(:,1)'*k*dk + gg(:,2)'*k*dk;
            Sp = excess_supply(nr, r, zz, kk, Df, Db, V, A_switch, dk, k);
            S_prime = (Sp - S(nr))/(0.0001);
            r = r - (S(nr)/S_prime)
...
% 
% set(gca, 'FontSize', 18)
% plot(k, c_r(:,1,nr), 'LineWidth', 2, 'LineStyle', '-', 'Color', 'r')
% hold on
% plot(k, c_r(:,2,nr), 'LineWidth', 2, 'LineStyle', '-', 'Color', 'b')
% hold off
% grid
% xlabel('Wealth, k','FontSize', 14)
...
% 
% set(gca, 'FontSize', 18)
% plot(k, adot(:,1,nr), 'LineWidth', 2, 'LineStyle', '-', 'Color', 'r')
% hold on
% plot(k, adot(:,2,nr), 'LineWidth', 2, 'LineStyle', '-', 'Color', 'b')
% hold off
% grid
% xlabel('Wealth, k', 'FontSize', 14)
...
% 
% set(gca, 'FontSize', 18)
% plot(k, V_r(:,1,nr), 'LineWidth', 2, 'LineStyle', '-', 'Color', 'r')
% hold on
% plot(k, V_r(:,2,nr), 'LineWidth', 2, 'LineStyle', '-', 'Color', 'b')
% hold off
% grid
% xlabel('Wealth, k', 'FontSize', 14)
...
% 
% set(gca, 'FontSize', 14)
% plot(k, g_r(:,1,nr), 'LineWidth', 2, 'LineStyle', '-', 'Color', 'r')
% hold on
% plot(k, g_r(:,2,nr), 'LineWidth', 2, 'LineStyle', '-', 'Color', 'b')
% hold off
% grid
% xlabel('Wealth, k', 'FontSize', 14)
...
% yy = get(gca, 'yLim');
% hold on
% plot([p.kmin, p.kmin], yy, '--k', 'LineWidth', 2)
% hold off
% text(-0.15, yy(1)-0.02*(yy(2) - yy(1)), '$\underline{k}$', 'HorizontalAlignment', 'center', 'FontSize', 15, 'Interpreter', 'latex')
% xlim([-0.2 1])
% legend(sprintf('Unemployed, r=%.4f', r), ...
%        sprintf('Employed, r=%.4f', r), 'Location', 'best', 'FontSize', 14)